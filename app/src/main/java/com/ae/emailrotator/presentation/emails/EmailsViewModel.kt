...
class EmailsViewModel @Inject constructor(
    ...
    fun showUpdateLimit(email: Email) = _state.update { it.copy(updateLimitEmail = email) }
    fun dismissUpdateLimit() = _state.update { it.copy(updateLimitEmail = null) }
    ...
    fun saveEmail(address: String, needsVerification: Boolean) {
        viewModelScope.launch {
            ...
            runCatching {
                if (editing != null) {
                    updateEmail(editing.globalEmailId, address)
                    _state.update {
    ...
    fun updateLimitTime(statusId: Long, newAvailableAt: Long) {
        viewModelScope.launch {
            ...
            _state.update { it.copy(updateLimitEmail = null, snackbar = "Limit time updated.") }
        }
    }
